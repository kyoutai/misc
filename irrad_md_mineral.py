...
par.add_argument('-d', '--dump', default=1000,
                 type=int, help="dump every this many timesteps")
par.add_argument('-p', '--press', default=1.0,
                 type=float, help="puressure setting")
par.add_argument('-t', '--temp', default=300,
                 type=int, help="tempreture")
par.add_argument('--pot_file', default="ffieldCSH.reax",
                 help="using potential file")
par.add_argument('-m', '--mode', choices=['quartz', 'albite', 'orthoclase', 'plagioclase', 'diopside', 'forsterite', 'augite'],
                 default='quartz', help="select mineral")
...
pair_coeff      * * {pot_file} {elems}
fix             1 all qeq/reax 1 0.0 10.0 1.0e-6 reax/c
"""

FORCE_BKSZBL = """pair_style      table spline 100000 ewald
"""

FORCE_BKSZBL2 = """pair_coeff              {id1} {id2} spline_{atom1}_{atom2}.pair buck_zbl_spline
"""
...
LAMMPSIN_INIT = """
velocity        all create {temp} 777
fix	        10 all npt temp {temp} {temp} 100 {system} {press} {press} 1000
dump            1 all custom {dump} {base}.bin id type element x y z vx vy vz
...
fix             11 all npt temp {temp} {temp} 1000000000000 {system} {press} {press} 1000
...
label           loop
  compute         3 {compute_disp} displace/atom
  compute         4 {compute_disp} reduce max c_3[4]
...
group sink      type {types_1}
group damage    type {types_2}

fix             10 all npt temp {temp} {temp} 100 {system} {press} {press} 1000
fix             12 sink temp/berendsen {temp} {temp} 100
...
class CIFDATA():
    ...
    def GetData(self):
        ...
        #data = re.search("(   Si1 .+)loop_.+",data, re.DOTALL)[1]
        data = data.strip()
...
class LammpsData():
    ...
    def Cif2Data(self, ciffile):
        ...
        if np.round(alpha * 180 / np.pi) == 90 and\
           ...
           np.round(gamma * 180 / np.pi) == 90:
            self.box = np.array([[0.0, a], [0.0, b], [0.0, c]])
            self.system = "aniso"
        else:  # triclnic
            lx = a
            xy = b * np.cos(gamma)
    ...
    def GetBinData2(self, offset, f):
        ...
        if ntimestep < 0:
            ...
            step = struct.unpack("<q", f.read(8))[0] # step
    ...
    def Bin2Data(self, restart):
        ...
        if args.bin_new is False:
            ...
            size, M, data = self.GetBinData2(0, f)        
        steps = int(filesize / size)
        # 最終ステップのデータ
        if args.bin_new is False:
            size, M, data = self.GetBinData(size*(steps - 1), f)
        else:
            size, M, data = self.GetBinData2(size*(steps - 1), f)
        atoms = data.shape[0]
    ...
    def UpdateTypeCharge(self):
        ...
        self.masses[:, 1:2] = self.masses[:, 1:2].astype(float)
    ...
    def WriteLammpsIn(self):
        ...
        # atom_style Bertrandの力場を使うものはfull, それ以外はcharge
        atom_style = "charge"
        if args.bertrand is True:
            atom_style = "full"
        ...
        if args.cascade is not None:
            ...
            if args.mode == "plagioclase":
                disp = "group disp      type 3 5          # 動きやすいので,dispの判定から除く"
                non_disp = "group non-disp  subtract all disp"
                compute_disp = "non-disp"
            elif args.mode == "quartz":
                disp = ""
                non_disp = ""
                compute_disp = "all"
            elif args.mode == "diopside":
                disp = "group disp      type 3 4          # 動きやすいので,dispの判定から除く"
                non_disp = "group non-disp  subtract all disp"
                compute_disp = "non-disp"
            elif args.mode == "augite":
                disp = "group disp      type 3 4 5         # 動きやすいので,dispの判定から除く"
                non_disp = "group non-disp  subtract all disp"
                compute_disp = "non-disp"

            else:
                disp = "group disp      type 3            # 動きやすいので,dispの判定から除く"
                non_disp = "group non-disp  subtract all disp"
                compute_disp = "non-disp"
            ...
            lammps_init = lammps_init.format(base=args.basename,
                                             ...
                                             temp=args.temp,
                                             group_disp=disp,
                                             group_non_disp=non_disp,
                                             compute_disp=compute_disp,
                                             system=self.system)
        if args.equilib is not None:
            ...
            types_2 = ""
            types = self.masses[:, 0]
            for i in range(int(types.shape[0]/2)):
                types_1 += "{} ".format(int(types[i]))
                types_2 += "{} ".format(int(types[i]+types.shape[0]/2))
            lammps_init = lammps_init.format(base=args.basename,
                                             elems=elems,
                                             dump=args.dump,
                                             press=args.press,
        ...
        for i in range(masses.shape[0]):
            masses[i] = masses[i].lower()
        if args.mode == 'quartz':
            o.write(FORCE_REAX.format(elems=elems, pot_file=args.pot_file))
        else:
            if args.cascade is not None:
                types_num = types.shape[0]
            if args.equilib is not None:
                types_num = types.shape[0]/2
            o.write(FORCE_BKSZBL)
            for i in types:
                for j in types:
                    if i <= j:
                        atom1 = masses[i-1]
                        atom2 = masses[j-1]
                        if j > types_num:
                            if i <= types_num and i > j - types_num:
                                atom1 = masses[j-1]
                                atom2 = masses[i-1]
                            if i > types_num and i - types_num > j - types_num:
                                atom1 = masses[j-1]
                                atom2 = masses[i-1]
                        o.write(FORCE_BKSZBL2.format(id1=i,
                                                     id2=j,
                                                     atom1=atom1,
                                                     atom2=atom2))
            o.write(NEIGHBOR_BKSZBL)
        o.write(lammps_init)
        o.close()
