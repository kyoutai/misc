...
# lammpstrj (原子の軌跡)からXRDピークを導出した後、
# plumed.dat からXRDピークを導出した結果ファイル COLVAR をプロットして、
# python で設計したXRDピーク導出計算が plumed でも正常に実現できたか確認する。
# XRD.py readXRDpeak.py の組合せプログラムです。
...
class XRD():
    def __init__(self, frname):
        ...
        # morch_std = np.std(self.morch, axis=0)
        ax.plot(Ttheta, foo_ave, "b--", label="without Lorch func.")
        ax.plot(Ttheta, morch_ave, "g-", label="Lorch func.")
        # ax.fill_between(Ttheta, morch_ave-morch_std, morch_ave+morch_std)
        ax.set_xlabel('2theta')
        ax.set_ylabel('Intensity')
        ax.legend()


# まず、lammpstrj を読み込んでXRDピークをプロット
a = XRD(args.file)
# そして、COLVAR を読み込んでXRDピークをプロット
