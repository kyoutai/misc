    ...
    def __init__(self, number, frname):
        ...
        for IDX, i in enumerate(range(start, end)):
            ...
            for idx, q in enumerate(Q):
                ...
                self.morch[IDX, idx] += coeff * self.atoms/3
                # non_Lorch
                # self.foo[IDX, idx] += np.sum(np.sin(x) / x) * coeff
                # self.foo[IDX, idx] += coeff * self.atoms/3
        ...
        # for idx, q in enumerate(Q):
        #     self.morch[idx] = foo_ave[idx] * Lorch(q, Qmax)
        ...
        ax.plot(Ttheta, morch_ave, mark[number], label=self.base+"L")
